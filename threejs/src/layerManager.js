...
/**
 * This class is responsible for managing the thematic layers of the map. It reacts to two sources of input:
 * 1. When layers are turned on or off via the thematic layers menu.
 * 2. When switching into or away from building view.
 * 
 * Additionally this class creates labels (and potentially icons) when loading into the map.
 * 
 * The UI elements of the thematic layers menu are also generated by this class.
 */
...
export class LayerManager {
	...
	constructor(scene, iconsSceneManager, svgLoader) {
		...
		/**
		* This array can be populated with layer codes (such as "He-SD") to allow these layers to be visible from
		* map startup.
		*/
		this.active_layers = [];

		/**
		* This value determines which importance value is sufficient to warrant getting a label.
		* It can be updated during map operation with the function update_importance_baseline(),
		* which takes as input the new importance value.
		*/
		this.importance_baseline = "Tertiary";

		/**
		* This hex color value determines what color icons will be given if they do not have a
		* color defined in the thematic_codelist-definition.json file.
		*/
		this.standard_icon_color = "#b2dbf2";

		/**
		* The above three variables should probably be moved to a separate file, as they are important
		* for determining the visuals of the map as it appears when first openened.
		*/
	 ...
	 * Unhide all the parents of unit meshes and hide the meshes themselves
	 *
	 * @param {THREE.Object3D} threejsObject: The current object being evaluated.
	 */
	 ...
	 * 
	 * This function removes a specific iconset, and not all iconsets / icons belonging to a specific
	 * thematic type. It is used when removing a label and icons for a building when building view is activated.
	 */
	 ...
	 * and the label of the active building will be restored. This is done when leaving building view.
	 * If clear_active_values is false, it indicates that the function is being used to switch between stories.
	 * As when this is the case it is necessary to clear labels, but not to clear the information relevant
	 * to the current building, as we remain in building view.
	 *  
	 */
	 ...
	 * 
	 * This function determines what needs to be activated for the current layer.
	 * It is currently possible for layers to have both an icon and geometry, or only one of these.
	 */
	...
	/**
	 * @param {string} layer_code: The layer code for which needs to be checked if it is active.
	 * 
	 * This function returns a boolean indicating if the indicated layer is currently active.
	 * 
	 * @returns A boolean describing if the layer in question is active.
	 */
	 ...
	 * @param {string} layer_code: The layer code to check.
	 * 
	 * Returns true or false depending on if the layer is an icon one or not.
	 * 
	 * @returns A boolean describing if the layer in question is an icon one.
	 */
	...
	/**
	 * This function populates the div reserved for the layers dropdown menu.
	 * The structure of this menu is defined in the file thematic_codelist-hierarchy.json.
	 * 
	 * Changes to which layers are active are done by calling _update_active_layers() with the
	 * layer code that needs to be toggled.
	 */
	 ...
	 * 
	 * Returns an image object using the given path.
	 * 
	 * @returns An html image object.
	 */
	_make_icon_img(path) {
	 ...
	 * @param {string} layer_code: The code of the layer.
	 * @param {object} itemsContainer_object: The object that the entry needs to be added to.
	 * @param {object} updateGroupCheckbox: Object that handles the menu checkboxes.
	 * 
	 * Creates a single thematic layers menu entry.
	 */
