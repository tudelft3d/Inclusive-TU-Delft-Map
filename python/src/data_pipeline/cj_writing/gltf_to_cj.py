...
import numpy as np
import trimesh
from data_pipeline.cj_helpers.cj_attributes import BdgUnitAttrReader
from data_pipeline.cj_helpers.cj_geometry import Geometry, MultiSurface
...
def _get_scene_geometry_from_id(scene: trimesh.Scene, cj_key: str) -> trimesh.Trimesh:
    """
    Extract the geometry of an object from the scene based on its ID.
    The geometry is processed in different ways to try to fix potential issues:
    - Swap the axes, assuming that the file comes from Blender and has Y up,
    - Removing duplicate vertices and triangles,
    - Removing small triangles and slivers,
    - Try to orient them consistently,
    - Try to fill the holes.

    Parameters
    ----------
    scene : trimesh.Scene
        The scene to extract the object geometry from.
    cj_key : str
        The key of the object in the scene.
...
) -> tuple[MultiSurface, str]:
    """
    Extract the geometry of an object from the scene based on its ID.
    Return its geometry and its name.
...
def _get_unit_geometry_from_id(scene: trimesh.Scene, cj_key: str) -> MultiSurface:
    mesh = _get_scene_geometry_from_id(scene=scene, cj_key=cj_key)
    orient_polygons_z_up(mesh)
